...
export default async function NewCurtainAccessoryLineItemPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const quoteId = Number(id);
  if (!Number.isInteger(quoteId)) notFound();

  const config = getAccessoryFamilyConfig("curtain_accessory");
  if (!config) notFound();
